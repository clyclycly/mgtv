...
<script type="text/ecmascript-6">
  export default {
    ...
    created() {
      ...
      if (this.rMsg) {
        let dispose = []
        this.againImg.length = this.rMsg.moduleData.length
        for (let i = 0; i < this.rMsg.moduleData.length; i++) {
          dispose.push({
            indexes: this.rMsg.moduleData[i].playerType,
            img: this.rMsg.moduleData[i].phoneImgUrl || this.rMsg.moduleData[i].imgHUrl,
            text: this.rMsg.moduleData[i].name
          })
        }
